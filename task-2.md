...
| Piece | Location |
|--------|----------|
| Gateway entrypoint + env injection | `nanobot/entrypoint.py` |
| `nanobot` + `webchat` Docker image | `nanobot/Dockerfile` |
| `nanobot` + `client-web-flutter` + Caddy wiring | `docker-compose.yml`, `caddy/Caddyfile` |
| Dependencies (`nanobot-ai`, `lms-mcp`, `nanobot-webchat` from Git) | `nanobot/pyproject.toml` |

### Local (once)

From the repo root, add the WebSocket channel repo (required for the Flutter build context):

```bash
git submodule add https://github.com/inno-se-toolkit/nanobot-websocket-channel.git nanobot-websocket-channel
git submodule update --init --recursive
```

If the folder already exists, skip `git submodule add`. Then resolve Python deps for local CLI use:
...
WebSocket test (install `websocat` if needed: `cargo install websocat` or your package manager):

```bash
echo '{"content":"What labs are available?"}' | websocat "ws://localhost:42002/ws/chat?access_key=YOUR_NANOBOT_ACCESS_KEY"
```

Browser: `http://<vm-ip>:42002/flutter` — use the same `NANOBOT_ACCESS_KEY` as in `.env.docker.secret`.

Paste a short nanobot log line into `REPORT.md` → `## Task 2A — Deployed agent`, and a Flutter screenshot → `## Task 2B — Web client`.
