...
| Piece | Location |
|--------|----------|
| **Planted bug fix** | `backend/app/routers/items.py` — `GET /items/` no longer maps every exception to **404** `"Items not found"`; database and other failures now surface as real errors (e.g. **500** via the global handler). |
| Skill guidance for investigations | `nanobot/workspace/skills/observability/SKILL.md` (section on **"What went wrong?"** / health) |

### VM — workflow

1. **Part A:** Enhance skill if needed (already covers log → trace). Stop postgres: `docker compose --env-file .env.docker.secret stop postgres`. In Flutter (same session), trigger a request, then ask **"What went wrong?"** — paste answer into `REPORT.md` → `## Task 4A`.
2. **Part B:** In the **same** Flutter tab (no refresh), ask the agent to create the **2-minute** cron health check as in the task. List jobs, wait for a proactive message, screenshot → `## Task 4B`. Remove the test job; `docker compose --env-file .env.docker.secret start postgres`.
3. **Part C:** `docker compose --env-file .env.docker.secret build backend && docker compose --env-file .env.docker.secret up -d`. Repeat failure test (stop postgres, request, **"What went wrong?"**) — response should show the **real** DB/connectivity error. Restart postgres; optional new cron; healthy report → `## Task 4C`.

If the stack was rebuilt, ensure **nanobot** image is current if you changed MCP/skills: `docker compose --env-file .env.docker.secret build nanobot && docker compose --env-file .env.docker.secret up -d`.
